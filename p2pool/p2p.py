...
class Protocol(p2protocol.Protocol):
    ...
    message_version = pack.ComposedType([
        ...
        ('addr_to', kzc_data.address_type),
        ('addr_from', kzc_data.address_type),
    ...
    def handle_version(self, version, services, addr_to, addr_from, nonce, sub_version, mode, best_share_hash):
        ...
        def update_remote_view_of_my_mining_txs(before, after):
            ...
            if removed:
                ...
                self.remote_remembered_txs_size -= sum(100 + kzc_data.tx_type.packed_size(before[x]) for x in removed)
            if added:
                self.remote_remembered_txs_size += sum(100 + kzc_data.tx_type.packed_size(after[x]) for x in added)
        ...
        self.remote_remembered_txs_size += sum(100 + kzc_data.tx_type.packed_size(x) for x in self.node.mining_txs_var.value.values())
        assert self.remote_remembered_txs_size <= self.max_remembered_txs_size
    ...
    def sendShares(self, shares, tracker, known_txs, include_txs_with=[]):
        ...
        new_remote_remembered_txs_size = self.remote_remembered_txs_size + sum(100 + kzc_data.tx_type.packed_size(known_txs[x]) for x in hashes_to_send)
        ...
        self.remote_remembered_txs_size -= sum(100 + kzc_data.tx_type.packed_size(known_txs[x]) for x in hashes_to_send)
    ...
    message_bestblock = pack.ComposedType([
        ('header', kzc_data.block_header_type),
    ])
    ...
    message_remember_tx = pack.ComposedType([
        ('tx_hashes', pack.ListType(pack.IntType(256))),
        ('txs', pack.ListType(kzc_data.tx_type)),
    ])
    def handle_remember_tx(self, tx_hashes, txs):
        for tx_hash in tx_hashes:
            ...
            self.remembered_txs_size += 100 + kzc_data.tx_type.packed_size(tx)
        added_known_txs = {}
        warned = False
        for tx in txs:
            tx_hash = kzc_data.hash256(kzc_data.tx_type.pack(tx))
            if tx_hash in self.remembered_txs:
                print >>sys.stderr, 'Peer referenced transaction twice, disconnecting'
                self.disconnect()
                return
    ...
    def handle_forget_tx(self, tx_hashes):
        for tx_hash in tx_hashes:
            self.remembered_txs_size -= 100 + kzc_data.tx_type.packed_size(self.remembered_txs[tx_hash])
