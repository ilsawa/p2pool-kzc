...
import kzc.getwork as kzc_getwork, kzc.data as kzc_data
from kzc import helper, script, worker_interface
...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def __init__(self, node, my_pubkey_hash, donation_percentage, merged_urls, worker_fee, args, pubkeys, kzcd):
        ...
        def compute_work():
            ...
            if bb is not None and bb['previous_block'] == t['previous_block'] and self.node.net.PARENT.POW_FUNC(kzc_data.block_header_type.pack(bb)) <= t['bits'].target:
                ...
                t = dict(
                    ...
                    transactions=[],
                    transaction_fees=[],
                    merkle_link=kzc_data.calculate_merkle_link([None], 0),
                    subsidy=self.node.kzcd_work.value['subsidy'],
                    last_update=self.node.kzcd_work.value['last_update'],
    ...
    def freshen_addresses(self, c):
        ...
        self.address = yield deferral.retry('Error getting a dynamic address from kzcd:', 5)(lambda: self.kzcd.rpc_getnewaddress('p2pool'))()
        new_pubkey = kzc_data.address_to_pubkey_hash(self.address, self.net)
        self.pubkeys.popleft()
        self.pubkeys.addkey(new_pubkey)
        print " Updated payout pool:"
        for i in xrange(len(self.pubkeys.keys)):
            print '    ...payout %d: %s(%f)' % (i, kzc_data.pubkey_hash_to_address(self.pubkeys.keys[i], self.net),self.pubkeys.keyweights[i],)
    ...
    def get_user_details(self, username):
        ...
        for symbol, parameter in zip(contents2[::2], contents2[1::2]):
            if symbol == '+':
                try:
                    desired_pseudoshare_target = kzc_data.difficulty_to_target(float(parameter))
                except:
                    if p2pool.DEBUG:
                        log.err()
            elif symbol == '/':
                try:
                    desired_share_target = kzc_data.difficulty_to_target(float(parameter))
                except:
                    if p2pool.DEBUG:
                        log.err()        
        ...
        if random.uniform(0, 100) < self.worker_fee:
            ...
            try:
                pubkey_hash = kzc_data.address_to_pubkey_hash(user, self.node.net.PARENT)
            except: # XXX blah
                if self.args.address != 'dynamic':
                    pubkey_hash = self.my_pubkey_hash
    ...
    def preprocess_request(self, user):
        ...
        if time.time() > self.current_work.value['last_update'] + 60:
            raise jsonrpc.Error_for_code(-12345)(u'lost contact with kzcd')
    ...
    def get_work(self, pubkey_hash, desired_share_target, desired_pseudoshare_target):
        ...
        if self.merged_work.value:
            tree, size = kzc_data.make_auxpow_tree(self.merged_work.value)
            mm_hashes = [self.merged_work.value.get(tree.get(i), dict(hash=0))['hash'] for i in xrange(size)]
            mm_data = '\xfa\xbemm' + kzc_data.aux_pow_coinbase_type.pack(dict(
                merkle_root=kzc_data.merkle_hash(mm_hashes),
                size=size,
                nonce=0,
            ))
        ...
        tx_hashes = [kzc_data.hash256(kzc_data.tx_type.pack(tx)) for tx in self.current_work.value['transactions']]
        ...
        if desired_share_target is None:
            ...
            if local_hash_rate > 0.0:
                desired_share_target = min(desired_share_target,
                    kzc_data.average_attempts_to_target(local_hash_rate * self.node.net.SHARE_PERIOD / 0.0167)) # limit to 1.67% of pool shares by modulating share difficulty
            lookbehind = 3600//self.node.net.SHARE_PERIOD
            block_subsidy = self.node.kzcd_work.value['subsidy']
            if previous_share is not None and self.node.tracker.get_height(previous_share.hash) > lookbehind:
                ...
                if expected_payout_per_block < self.node.net.PARENT.DUST_THRESHOLD:
                    desired_share_target = min(desired_share_target,
                        kzc_data.average_attempts_to_target((kzc_data.target_to_average_attempts(self.node.kzcd_work.value['bits'].target)*self.node.net.SPREAD)*self.node.net.PARENT.DUST_THRESHOLD/block_subsidy)
                    )
        ...
        packed_gentx = kzc_data.tx_type.pack(gentx)
        ...
        if desired_pseudoshare_target is None:
            ...
            if local_hash_rate is not None:
                target = min(target,
                    kzc_data.average_attempts_to_target(local_hash_rate * 1)) # limit to 1 share response every second by modulating pseudoshare difficulty
        else:
        ...
        merkle_link = kzc_data.calculate_merkle_link([None] + other_transaction_hashes, 0)
        ...
        if print_throttle is 0.0:
            ...
            if (current_time - print_throttle) > 5.0:
                print 'New work for worker %s! Difficulty: %.06f Share difficulty: %.06f Block %s Total value: %.6f %s including %i transactions' % (
                    kzc_data.pubkey_hash_to_address(pubkey_hash, self.node.net.PARENT),
                    kzc_data.target_to_difficulty(target),
                    kzc_data.target_to_difficulty(share_info['bits'].target),
                    self.current_work.value['height'],
        ...
        #need this for stats
        self.last_work_shares.value[kzc_data.pubkey_hash_to_address(pubkey_hash, self.node.net.PARENT)]=share_info['bits']
        ...
        def got_response(header, user, coinbase_nonce):
            ...
            new_gentx = kzc_data.tx_type.unpack(new_packed_gentx) if coinbase_nonce != '\0'*self.COINBASE_NONCE_LENGTH else gentx

            header_hash = self.node.net.PARENT.BLOCKHASH_FUNC(kzc_data.block_header_type.pack(header))
            pow_hash = self.node.net.PARENT.POW_FUNC(kzc_data.block_header_type.pack(header))
            try:
                if pow_hash <= header['bits'].target or p2pool.DEBUG:
                    helper.submit_block(dict(header=header, txs=[new_gentx] + other_transactions), False, self.node.factory, self.node.kzcd, self.node.kzcd_work, self.node.net)
            ...
            assert header['merkle_root'] == kzc_data.check_merkle_link(kzc_data.hash256(new_packed_gentx), merkle_link)
                ...
                try:
                    if pow_hash <= aux_work['target'] or p2pool.DEBUG:
                        df = deferral.retry('Error submitting merged block: (will retry)', 10, 10)(aux_work['merged_proxy'].rpc_getauxblock)(
                            ...
                            kzc_data.aux_pow_type.pack(dict(
                                merkle_tx=dict(
                                    tx=new_gentx,
                                    block_hash=header_hash,
                                    merkle_link=merkle_link,
                                ),
                                merkle_link=kzc_data.calculate_merkle_link(hashes, index),
            ...
            if pow_hash <= share_info['bits'].target and header_hash not in received_header_hashes:
                ...
                self.share_received.happened(kzc_data.target_to_average_attempts(share.target), not on_time, share.hash)
            ...
            if pow_hash > target:
                ...
                self.pseudoshare_received.happened(kzc_data.target_to_average_attempts(target), not on_time, user)
                self.recent_shares_ts_work.append((time.time(), kzc_data.target_to_average_attempts(target)))
                while len(self.recent_shares_ts_work) > 50:
                    self.recent_shares_ts_work.pop(0)
                self.local_rate_monitor.add_datum(dict(work=kzc_data.target_to_average_attempts(target), dead=not on_time, user=user, share_target=share_info['bits'].target))
                self.local_addr_rate_monitor.add_datum(dict(work=kzc_data.target_to_average_attempts(target), pubkey_hash=pubkey_hash))
