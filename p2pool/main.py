...
import kzc.p2p as kzc_p2p, kzc.data as kzc_data
from kzc import stratum, worker_interface, helper
...
class keypool():
    ...
    def paytotal(self):
        ...
        for i in xrange(len(pubkeys.keys)):
            self.payouttotal += node.get_current_txouts().get(kzc_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)*1e-8
...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to kzcd over kzc-p2p
            print '''Testing kzcd P2P connection to '%s:%s'...''' % (args.kzcd_address, args.kzcd_p2p_port)
            factory = kzc_p2p.ClientFactory(net.PARENT)
            reactor.connectTCP(args.kzcd_address, args.kzcd_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of kzcd's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so kzcd can work without connections
            factory = yield connect_p2p()
        
        # connect to kzcd over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.kzcd_rpc_ssl else 'http', args.kzcd_address, args.kzcd_rpc_port)
        print '''Testing kzcd RPC connection to '%s' with username '%s'...''' % (url, args.kzcd_rpc_username)
        kzcd = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.kzcd_rpc_username + ':' + args.kzcd_rpc_password)), timeout=30)
        yield helper.check(kzcd, net)
        temp_work = yield helper.getwork(kzcd, net)
        
        kzcd_getinfo_var = variable.Variable(None)
        @defer.inlineCallbacks
        def poll_warnings():
            kzcd_getinfo_var.set((yield deferral.retry('Error while calling getinfo:')(kzcd.rpc_getinfo)()))
        yield poll_warnings()
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                res = yield deferral.retry('Error validating cached address:', 5)(lambda: kzcd.rpc_validateaddress(address))()
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local kzcd!'
                    address = None
            
            if address is None:
                print '    Getting payout address from kzcd...'
                address = yield deferral.retry('Error getting payout address from kzcd:', 5)(lambda: kzcd.rpc_getaccountaddress('p2pool'))()
            
            with open(address_path, 'wb') as f:
                f.write(address)
            
            my_pubkey_hash = kzc_data.address_to_pubkey_hash(address, net.PARENT)
            print '    ...success! Payout address:', kzc_data.pubkey_hash_to_address(my_pubkey_hash, net.PARENT)
            print
            ...
            for i in xrange(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from kzcd:', 5)(lambda: kzcd.rpc_getnewaddress('p2pool'))()
                new_pubkey = kzc_data.address_to_pubkey_hash(address, net.PARENT)
            ...
            for i in xrange(len(pubkeys.keys)):
                print '    ...payout %d: %s' % (i, kzc_data.pubkey_hash_to_address(pubkeys.keys[i], net.PARENT),)
        ...
        node = p2pool_node.Node(factory, kzcd, shares.values(), known_verified, net)
        yield node.start()
        ...
        wb = work.WorkerBridge(node, my_pubkey_hash, args.donation_percentage, merged_urls, args.worker_fee, args, pubkeys, kzcd)
        web_root = web.get_web_root(wb, datadir_path, kzcd_getinfo_var, static_dir=args.web_static)
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (net.NAME.upper(), kzc_data.script2_to_address(share.new_script, net.PARENT), net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/kzc_data.target_to_average_attempts(datum['share_target']) for datum in datums)
                    ...
                    if height > 2:
                        ...
                        paytot = 0.0
                        for i in xrange(len(pubkeys.keys)):
                            curtot = node.get_current_txouts().get(kzc_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)
                            paytot += curtot*1e-8
                        ...
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            ...
                            100*stale_prop,
                            math.format_dt(2**256 / node.kzcd_work.value['bits'].target / real_att_s),
                        )
                        
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net, kzcd_getinfo_var.value, node.kzcd_work.value):
                            print >>sys.stderr, '#'*40
...
def run():
    ...
    parser.add_argument('--net',
        help='use specified network (default: kzc)',
        action='store', choices=sorted(realnets), default='kzc', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from kzcd>), or (dynamic)',
        type=str, action='store', default=None, dest='address')
    parser.add_argument('-i', '--numaddresses',
        help='number of kzc auto-generated addresses to maintain for getwork dynamic address allocation',
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own kzc address (by setting their miner's username to a kzc address) this percentage fee to mine on your p2pool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    kzcd_group = parser.add_argument_group('kzcd interface')
    kzcd_group.add_argument('--kzcd-config-path', metavar='KZCD_CONFIG_PATH',
        help='custom configuration file path (when kzcd -conf option used)',
        type=str, action='store', default=None, dest='kzcd_config_path')
    kzcd_group.add_argument('--kzcd-address', metavar='KZCD_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='kzcd_address')
    kzcd_group.add_argument('--kzcd-rpc-port', metavar='KZCD_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from kzc.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='kzcd_rpc_port')
    kzcd_group.add_argument('--kzcd-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='kzcd_rpc_ssl')
    kzcd_group.add_argument('--kzcd-p2p-port', metavar='KZCD_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from kzc.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='kzcd_p2p_port')
    
    kzcd_group.add_argument(metavar='KZCD_RPCUSERPASS',
        help='kzcd RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause P2Pool to read them from kzc.conf)',
        type=str, action='store', default=[], nargs='*', dest='kzcd_rpc_userpass')
    ...
    if len(args.kzcd_rpc_userpass) > 2:
        parser.error('a maximum of two arguments are allowed')
    args.kzcd_rpc_username, args.kzcd_rpc_password = ([None, None] + args.kzcd_rpc_userpass)[-2:]
    
    if args.kzcd_rpc_password is None:
        conf_path = args.kzcd_config_path or net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''kzc configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart kzc.''' % (conf_path, random.randrange(2**128)))
        ...
        for conf_name, var_name, var_type in [
            ('rpcuser', 'kzcd_rpc_username', str),
            ('rpcpassword', 'kzcd_rpc_password', str),
            ('rpcport', 'kzcd_rpc_port', int),
            ('port', 'kzcd_p2p_port', int),
        ]:
        ...
        if 'rpcssl' in contents and contents['rpcssl'] != '0':
                args.kzcd_rpc_ssl = True
        if args.kzcd_rpc_password is None:
            parser.error('''kzc configuration file didn't contain an rpcpassword= line! Add one!''')
    
    if args.kzcd_rpc_username is None:
        args.kzcd_rpc_username = ''
    
    if args.kzcd_rpc_port is None:
        args.kzcd_rpc_port = net.PARENT.RPC_PORT
    
    if args.kzcd_p2p_port is None:
        args.kzcd_p2p_port = net.PARENT.P2P_PORT
    ...
    if args.address is not None and args.address != 'dynamic':
        try:
            args.pubkey_hash = kzc_data.address_to_pubkey_hash(args.address, net.PARENT)
