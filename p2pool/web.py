...
import p2pool
from kzc import data as kzc_data
...
def get_web_root(wb, datadir_path, kzcd_getinfo_var, stop_event=variable.Event(), static_dir=None):
    node = wb.node
    ...
    def get_users():
        ...
        for script in sorted(weights, key=lambda s: weights[s]):
            res[kzc_data.script2_to_address(script, node.net.PARENT)] = weights[script]/total_weight
        return res
    ...
    def get_patron_sendmany(total=None, trunc='0.01'):
        ...
        return json.dumps(dict(
            (kzc_data.script2_to_address(script, node.net.PARENT), value/1e8)
            for script, value in get_current_scaled_txouts(total, trunc).iteritems()
            if kzc_data.script2_to_address(script, node.net.PARENT) is not None
        ))
    ...
    def get_global_stats():
        ...
        diff = kzc_data.target_to_difficulty(wb.current_work.value['bits'].target)
        return dict(
            pool_nonstale_hash_rate=nonstale_hash_rate,
            pool_hash_rate=nonstale_hash_rate/(1 - stale_prop),
            pool_stale_prop=stale_prop,
            min_difficulty=kzc_data.target_to_difficulty(node.tracker.items[node.best_share_var.value].max_target),
    ...
    def get_local_stats():
        ...
        my_work = sum(kzc_data.target_to_average_attempts(share.target)
        ...
        for addr in wb.last_work_shares.value:
            miner_last_difficulties[addr] = kzc_data.target_to_difficulty(wb.last_work_shares.value[addr].target)
        ...
        return dict(
            ...
            attempts_to_share=kzc_data.target_to_average_attempts(node.tracker.items[node.best_share_var.value].max_target),
            attempts_to_block=kzc_data.target_to_average_attempts(node.kzcd_work.value['bits'].target),
            block_value=node.kzcd_work.value['subsidy']*1e-8,
            warnings=p2pool_data.get_warnings(node.tracker, node.best_share_var.value, node.net, kzcd_getinfo_var.value, node.kzcd_work.value),
    ...
    web_root.putChild('difficulty', WebInterface(lambda: kzc_data.target_to_difficulty(node.tracker.items[node.best_share_var.value].max_target)))
    web_root.putChild('users', WebInterface(get_users))
    web_root.putChild('user_stales', WebInterface(lambda: dict((kzc_data.pubkey_hash_to_address(ph, node.net.PARENT), prop) for ph, prop in
    ...
    web_root.putChild('current_payouts', WebInterface(lambda: dict((kzc_data.script2_to_address(script, node.net.PARENT), value/1e8) for script, value in node.get_current_txouts().iteritems())))
    ...
    web_root.putChild('payout_addr', WebInterface(lambda: kzc_data.pubkey_hash_to_address(wb.my_pubkey_hash, node.net.PARENT)))
    web_root.putChild('payout_addrs', WebInterface(lambda: list(('%s' % kzc_data.pubkey_hash_to_address(add, node.net.PARENT)) for add in wb.pubkeys.keys)))
    ...
    def update_stat_log():
        ...
        for add in wb.pubkeys.keys:
            my_current_payout+=node.get_current_txouts().get(kzc_data.pubkey_hash_to_script2(add), 0)*1e-8
    ...
    def get_share(share_hash_str):
        ...
        return dict(
            ...
            share_data=dict(
                ...
                payout_address=kzc_data.script2_to_address(share.new_script, node.net.PARENT),
    ...
    def add_point():
        ...
        for add in wb.pubkeys.keys:
             my_current_payouts += current_txouts.get(kzc_data.pubkey_hash_to_script2(add), 0)*1e-8
        hd.datastreams['current_payout'].add_datum(t, my_current_payouts)
        miner_hash_rates, miner_dead_hash_rates = wb.get_local_rates()
        current_txouts_by_address = dict((kzc_data.script2_to_address(script, node.net.PARENT), amount) for script, amount in current_txouts.iteritems())
