...
from p2pool.kzc import data as kzc_data, helper, height_tracker
...
class P2PNode(p2p.Node):
    ...
    def handle_shares(self, shares, peer):
        ...
        for share, new_txs in shares:
            if new_txs is not None:
                all_new_txs.update((kzc_data.hash256(kzc_data.tx_type.pack(new_tx)), new_tx) for new_tx in new_txs)
    ...
    def handle_bestblock(self, header, peer):
        if self.node.net.PARENT.POW_FUNC(kzc_data.block_header_type.pack(header)) > header['bits'].target:
        ...
        def _(share):
            ...
            def spread():
                if (self.node.get_height_rel_highest(share.header['previous_block']) > -5 or
                    self.node.kzcd_work.value['previous_block'] in [share.header['previous_block'], share.header_hash]):
...
class Node(object):
    def __init__(self, factory, kzcd, shares, known_verified_share_hashes, net):
    ...
    def start(self):
        ...
        self.kzcd_work = variable.Variable((yield helper.getwork(self.kzcd, self.net)))
        @defer.inlineCallbacks
        def work_poller():
            while stop_signal.times == 0:
                flag = self.factory.new_block.get_deferred()
                try:
                    self.kzcd_work.set((yield helper.getwork(self.kzcd, self.net, self.kzcd_work.value['use_getblocktemplate'])))
                except:
                    log.err()
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(kzc_data.block_header_type.pack(new_header)) <= self.kzcd_work.value['bits'].target):
                return
            kzcd_best_block = self.kzcd_work.value['previous_block']
            if (self.best_block_header.value is None
                or (
                    new_header['previous_block'] == kzcd_best_block and
                    self.net.PARENT.BLOCKHASH_FUNC(kzc_data.block_header_type.pack(self.best_block_header.value)) == kzcd_best_block
                ) # new is child of current and previous is current
                or (
                    self.net.PARENT.BLOCKHASH_FUNC(kzc_data.block_header_type.pack(new_header)) == kzcd_best_block and
                    self.best_block_header.value['previous_block'] != kzcd_best_block
        ...
        def poll_header():
            if self.factory.conn.value is None:
                return
            handle_header((yield self.factory.conn.value.get_block_header(self.kzcd_work.value['previous_block'])))
        self.kzcd_work.changed.watch(lambda _: poll_header())
        ...
        self.get_height_rel_highest = yield height_tracker.get_height_rel_highest_func(self.kzcd, self.factory, lambda: self.kzcd_work.value['previous_block'], self.net)
        
        self.best_share_var = variable.Variable(None)
        self.desired_var = variable.Variable(None)
        self.kzcd_work.changed.watch(lambda _: self.set_best_share())
        self.set_best_share()
        
        # setup p2p logic and join p2pool network
        
        # update mining_txs according to getwork results
        @self.kzcd_work.changed.run_and_watch
        def _(_=None):
            new_mining_txs = {}
            added_known_txs = {}
            for tx_hash, tx in zip(self.kzcd_work.value['transaction_hashes'], self.kzcd_work.value['transactions']):
        ...
        # add p2p transactions from kzcd to known_txs
        @self.factory.new_tx.watch
        def _(tx):
            self.known_txs_var.add({
                kzc_data.hash256(kzc_data.tx_type.pack(tx)): tx,
            })
        # forward transactions seen to kzcd
        ...
        def _(share):
            ...
            if block is None:
                print >>sys.stderr, 'GOT INCOMPLETE BLOCK FROM PEER! %s kzc: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
                return
            helper.submit_block(block, True, self.factory, self.kzcd, self.kzcd_work, self.net)
            print
    ...
    def set_best_share(self):
        best, desired, decorated_heads, bad_peer_addresses = self.tracker.think(self.get_height_rel_highest, self.kzcd_work.value['previous_block'], self.kzcd_work.value['bits'], self.known_txs_var.value)
	...
	#i = 210240
	real_subsidy = self.kzcd_work.value['subsidy']
	#while i <= self.kzcd_work.value['height']:
		#real_subsidy = real_subsidy*92.9/100
		#i = i + 210240
        # Use value from getblocktemplate's result.
        if self.kzcd_work.value['payment_amount'] >= 0 :
            real_pay = real_subsidy - self.kzcd_work.value['payment_amount']
            return p2pool_data.get_expected_payouts(self.tracker, self.best_share_var.value, self.kzcd_work.value['bits'].target, real_pay, self.net)
            
	if self.kzcd_work.value['height'] > 158000+((576*30)* 17):
		real_pay = (real_subsidy)*40/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 15):
            real_pay = (real_subsidy)*42.5/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 13):
            real_pay = (real_subsidy)*45/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 11):
            real_pay = (real_subsidy)*47.5/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 9):
            real_pay = (real_subsidy)*50/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 7):
            real_pay = (real_subsidy)*52.5/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 6):
            real_pay = (real_subsidy)*55/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 5):
            real_pay = (real_subsidy)*57.5/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 4):
            real_pay = (real_subsidy)*60/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 3):
            real_pay = (real_subsidy)*62.5/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 2):
            real_pay = (real_subsidy)*65/100
        elif self.kzcd_work.value['height'] > 158000+((576*30)* 1):
