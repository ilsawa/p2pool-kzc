'''
Implementation of Kzc's p2p protocol
'''
...
class Protocol(p2protocol.Protocol):
    ...
    # https://github.com/kzcashteam/kzcash/blob/master/src/protocol.h#L338-L362
    ...
    message_block = pack.ComposedType([
        ('block', kzc_data.block_type),
    ])
    def handle_block(self, block):
        block_hash = self.net.BLOCKHASH_FUNC(kzc_data.block_header_type.pack(block['header']))
    ...
    message_headers = pack.ComposedType([
        ('headers', pack.ListType(kzc_data.block_type_old)),
    ])
    def handle_headers(self, headers):
        for header in headers:
            header = header['header']
            self.get_block_header.got_response(self.net.BLOCKHASH_FUNC(kzc_data.block_header_type.pack(header)), header)
    ...
    def connectionLost(self, reason):
        ...
        if hasattr(self, 'pinger'):
            self.pinger.stop()
        if p2pool.DEBUG:
            print >>sys.stderr, 'Kzcd connection lost. Reason:', reason.getErrorMessage()
