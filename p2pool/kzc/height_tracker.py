...
class HeaderWrapper(object):
    ...
    def from_header(cls, header):
        return cls(kzc_data.hash256(kzc_data.block_header_type.pack(header)), header['previous_block'])
...
@defer.inlineCallbacks
def get_height_rel_highest_func(kzcd, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(kzcd.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield kzcd.rpc_getblock('%x' % (block_hash,))
