...
import p2pool
from p2pool.kzc import data as kzc_data, script, sha256
...
class Share(object):
    ...
    small_block_header_type = pack.ComposedType([
        ...
        ('bits', kzc_data.FloatingIntegerType()),
    ...
    share_info_type = pack.ComposedType([
        ...
        ('max_bits', kzc_data.FloatingIntegerType()),
        ('bits', kzc_data.FloatingIntegerType()),
        ('timestamp', pack.IntType(32)),
    ...
    def generate_transaction(cls, tracker, share_data, block_target, desired_timestamp, desired_target, ref_merkle_link, desired_other_transaction_hashes_and_fees, net, known_txs=None, last_txout_nonce=0, base_subsidy=None):
        ...
        max_bits = kzc_data.FloatingInteger.from_target_upper_bound(pre_target3)
        bits = kzc_data.FloatingInteger.from_target_upper_bound(math.clip(desired_target, (pre_target3//30, pre_target3)))
        ...
        weights, total_weight, donation_weight = tracker.get_cumulative_weights(previous_share.share_data['previous_share_hash'] if previous_share is not None else None,
            ...
            65535*net.SPREAD*kzc_data.target_to_average_attempts(block_target),
        )
        ...
        if payments is not None:
            for obj in payments:
                pm_script = kzc_data.address_to_script2(obj['payee'],net.PARENT)
        ...
        this_script = kzc_data.pubkey_hash_to_script2(share_data['pubkey_hash'])
        ...
        share_info = dict(
            ...
            abswork=((previous_share.abswork if previous_share is not None else 0) + kzc_data.target_to_average_attempts(bits.target)) % 2**128,
        )
        ...
        def get_share(header, last_txout_nonce=last_txout_nonce):
            ...
            share = cls(net, None, dict(
                ...
                hash_link=prefix_to_hash_link(kzc_data.tx_type.pack(gentx)[:-32-8-4], cls.gentx_before_refhash),
    ...
    def get_ref_hash(cls, net, share_info, ref_merkle_link):
        return pack.IntType(256).pack(kzc_data.check_merkle_link(kzc_data.hash256(cls.ref_type.pack(dict(
    ...
    def __init__(self, net, peer_addr, contents):
        ...
        self.new_script = kzc_data.pubkey_hash_to_script2(self.share_data['pubkey_hash'])
        ...
        merkle_root = kzc_data.check_merkle_link(self.gentx_hash, self.merkle_link)
        self.header = dict(self.min_header, merkle_root=merkle_root)
        self.pow_hash = net.PARENT.POW_FUNC(kzc_data.block_header_type.pack(self.header))
        self.hash = self.header_hash = net.PARENT.BLOCKHASH_FUNC(kzc_data.block_header_type.pack(self.header))
    ...
    def check(self, tracker):
        ...
        if kzc_data.hash256(kzc_data.tx_type.pack(gentx)) != self.gentx_hash:
            raise ValueError('''gentx doesn't match hash_link''')
        
        if kzc_data.calculate_merkle_link([None] + other_tx_hashes, 0) != self.merkle_link:
    ...
    def should_punish_reason(self, previous_block, bits, tracker, known_txs):
        ...
        if other_txs is None:
            ...
            all_txs_size = sum(kzc_data.tx_type.packed_size(tx) for tx in other_txs)
...
class WeightsSkipList(forest.TrackerSkipList):
    ...
    def get_delta(self, element):
        from p2pool.kzc import data as kzc_data
        share = self.tracker.items[element]
        att = kzc_data.target_to_average_attempts(share.target)
...
class OkayTracker(forest.Tracker):
    def __init__(self, net):
        forest.Tracker.__init__(self, delta_type=forest.get_attributedelta_type(dict(forest.AttributeDelta.attrs,
            work=lambda share: kzc_data.target_to_average_attempts(share.target),
            min_work=lambda share: kzc_data.target_to_average_attempts(share.max_target),
        )))
        self.net = net
    ...
    def think(self, block_rel_height_func, previous_block, bits, known_txs):
        ...
        if p2pool.DEBUG:
            print 'Desire %i shares. Cutoff: %s old diff>%.2f' % (len(desired), math.format_dt(time.time() - timestamp_cutoff), kzc_data.target_to_difficulty(target_cutoff))
            for peer_addr, hash, ts, targ in desired:
                print '   ', None if peer_addr is None else '%s:%i' % peer_addr, format_hash(hash), math.format_dt(time.time() - ts), kzc_data.target_to_difficulty(targ), ts >= timestamp_cutoff, targ <= target_cutoff
...
def get_stale_counts(tracker, share_hash, lookbehind, rates=False):
    ...
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        res['good'] = res.get('good', 0) + kzc_data.target_to_average_attempts(share.target)
        s = share.share_data['stale_info']
        if s is not None:
            res[s] = res.get(s, 0) + kzc_data.target_to_average_attempts(share.target)
    if rates:
...
def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    weights, total_weight, donation_weight = tracker.get_cumulative_weights(best_share_hash, min(tracker.get_height(best_share_hash), net.REAL_CHAIN_LENGTH), 65535*net.SPREAD*kzc_data.target_to_average_attempts(block_target))
...
def get_desired_version_counts(tracker, best_share_hash, dist):
    ...
    for share in tracker.get_chain(best_share_hash, dist):
        res[share.desired_version] = res.get(share.desired_version, 0) + kzc_data.target_to_average_attempts(share.target)
    return res

def get_warnings(tracker, best_share, net, kzcd_getinfo, kzcd_work_value):
    res = []
    ...
    if kzcd_getinfo['errors'] != '':
        if 'This is a pre-release test build' not in kzcd_getinfo['errors']:
            res.append('(from kzcd) %s' % (kzcd_getinfo['errors'],))
    
    version_warning = getattr(net, 'VERSION_WARNING', lambda v: None)(kzcd_getinfo['version'])
    if version_warning is not None:
        res.append(version_warning)
    
    if time.time() > kzcd_work_value['last_update'] + 60:
        res.append('''LOST CONTACT WITH KZCD for %s! Check that it isn't frozen or dead!''' % (math.format_dt(time.time() - kzcd_work_value['last_update']),))
