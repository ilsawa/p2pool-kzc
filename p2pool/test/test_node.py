...
from p2pool.kzc import data as kzc_data, networks, worker_interface
from p2pool.util import deferral, jsonrpc, math, variable

class kzcd(object): # can be used as p2p factory, p2p protocol, or rpc jsonrpc proxy
    def __init__(self):
        ...
        self.headers = {0x132b9afeca5e9a2fdf4477338df6dcff1342300240bc70397c4bb: {
            ...
            'bits': kzc_data.FloatingInteger(bits=0x1b0d642e, target=0x44b9f20000000000000000000000000000000000000000000000L),
        }}
    ...
    def rpc_getblocktemplate(self, param):
        if param['mode'] == 'template':
            ...
            result = param['data']
            block = kzc_data.block_type.unpack(result.decode('hex'))
            ...
            if block['header']['previous_block'] != self.blocks[-1]:
                return False
            if kzc_data.hash256(result.decode('hex')) > block['header']['bits'].target:
                return False
            header_hash = kzc_data.hash256(kzc_data.block_header_type.pack(block['header']))
        ...
        for i in xrange(100):
            fee = i
            txs.append(dict(
                data=kzc_data.tx_type.pack(dict(version=1, tx_ins=[], tx_outs=[dict(value=fee, script='hello!'*100)], lock_time=0)).encode('hex'),
                fee=fee,
            ))
        return {
            "version" : 3,
...
class MiniNode(object):
    ...
    def start(cls, net, factory, kzcd, peer_ports, merged_urls):
        self = cls()
        
        self.n = node.Node(factory, kzcd, [], [], net)
        yield self.n.start()
