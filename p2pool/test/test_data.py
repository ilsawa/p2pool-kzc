...
class Test(unittest.TestCase):
    def test_hashlink1(self):
        for i in xrange(100):
            ...
            assert data.check_hash_link(x, '') == kzc_data.hash256(d)
    ...
    def test_hashlink2(self):
        for i in xrange(100):
            ...
            x = data.prefix_to_hash_link(d)
            assert data.check_hash_link(x, d2) == kzc_data.hash256(d + d2)
    ...
    def test_hashlink3(self):
        for i in xrange(100):
            ...
            assert data.check_hash_link(x, d3, d2) == kzc_data.hash256(d + d2 + d3)
